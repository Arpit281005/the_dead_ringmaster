...
export default function LiveTimer({
  ...
  // Start null so server and first client render match exactly; real value fills in post-mount.
  const [now, setNow] = useState<number | null>(null);

  useEffect(() => {
    setNow(Date.now());
    if (finishedAtIso) return;
  ...
  const elapsed = end === null ? 0 : Math.max(0, Math.floor((end - startedAt) / 1000));
